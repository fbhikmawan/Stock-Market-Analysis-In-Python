...
def predict_score_output(X_train,y_train,X_test,y_test,lab,cc,models, print_results = True):
    ...
    for k in cc:
        ...
        if print_results:
            ...
            print('Score = ' + str(round(models[-1][0].score(X_k_train,y_k_train),3)), ', Count = ' + str(len(X_k_train)))
            print()
            print('Test set ' + str(k) + ' results:')
            print('Score = ' + str(round(models[-1][0].score(X_k_test,y_k_test),3)), ', Count = ' + str(len(X_k_test)))
            print()
    print('Fitting Done')
...
pipe_PCA = make_pipeline(StandardScaler(), PCA(n_components = 15,random_state = 42))
pipe_PCA.fit(X)
...
train_permute = permute[:5000]
test_permute = permute[5000:]
X_train = X.iloc[train_permute]
X_test = X.iloc[test_permute]
y = stock_data_copy[prf+'Open'].copy()
y_train = y.iloc[train_permute]
y_test = y.iloc[test_permute]

init_model = []

predict_score_output(X_train,y_train,X_test,y_test,'Open',counter, init_model)
print()
init_model
...
for label in predict_labels:
    ...
    predict_score_output(X_train,y_train,X_test,y_test,label,counter,LR)
