...
def get_fft(df,lab):
    features = [x for x in df.columns if x.find(lab) != -1 and x.find('-1') == -1]
    data_list_to_fft = np.fft.fft(np.array(df[features])[0])
    data_list_to_fft = data_list_to_fft/max(abs(data_list_to_fft))
    di1 = [[lab + '_real_fft_' + str(i),[np.real(x)]] for i,x in enumerate(data_list_to_fft)]
    di2 = [[lab + '_imag_fft_' + str(i),[np.imag(x)]] for i,x in enumerate(data_list_to_fft)]
    di = dict(di1+di2)
    df_out = pd.DataFrame(di)
    return df_out
...
save_file_path = '.\\data_FFT\\30_week_weekly_data.csv'
from_save = True
load_file_path = '.\\data\\30_week_weekly_data.csv'
paths = glob(path_to_stocks)

data_points, rg_to_avg, n_samples = 75, 1, 10

prefix_fun = lambda a : 'week_' + str(a)+ '_avg_'
...
if not from_save:
    for i,path in enumerate([paths[idd] for idd in rnd_perm]):
        ...
        for idx in permute_idx:
            ...
            data_list_entry = pd.concat(individual_stock_data_list,axis = 1)
            for label in predict_labels:
                 data_list_entry = pd.concat([data_list_entry,get_fft(data_list_entry,label)],axis = 1)
            data_list.append(data_list_entry)
    
    main_stock_data_frame = pd.concat(data_list ,ignore_index = True)
else:
    main_stock_data_frame_load = pd.read_csv(load_file_path, index_col = 0)
    data_list = []
    for i in main_stock_data_frame_load.index:
        if i%100 == 0:
            print(str(i) + '/' + str(len(main_stock_data_frame_load)) )
        data_list_entry = main_stock_data_frame_load.iloc[i:i+1].reset_index(drop = True)
        for label in predict_labels:
            data_list_entry = pd.concat([data_list_entry,get_fft(data_list_entry,label)],axis = 1)
        data_list.append(data_list_entry)
    main_stock_data_frame = pd.concat(data_list ,ignore_index = True)
